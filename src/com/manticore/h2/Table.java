...
public class Table implements Comparable<Table> {
  ...
  public static Collection<Table> getTables(DatabaseMetaData metaData) throws SQLException {
    ...
    try {
      ...
      while (rs.next()) {
        String tableCatalog =
            rs.getString("TABLE_CAT"); // TABLE_CATALOG String => catalog name (may be null)
        ...
        String tableType =
            rs.getString(
                "TABLE_TYPE"); // TABLE_TYPE String => table type. Typical types are "TABLE",
        // "VIEW", "SYSTEM TABLE", "GLOBAL TEMPORARY", "LOCAL TEMPORARY",
        // "ALIAS", "SYNONYM".

        String remarks =
            rs.getString(
                "REMARKS"); // REMARKS String => explanatory comment on the table (may be null)
        String typeCatalog =
            rs.getString("TYPE_CAT"); // TYPE_CAT String => the types catalog (may be null)
        String typeSchema =
            rs.getString("TYPE_SCHEM"); // TYPE_SCHEM String => the types schema (may be null)
        String typeName = rs.getString("TYPE_NAME"); // TYPE_NAME String => type name (may be null)
        String selfReferenceColName =
            rs.getString(
                "SELF_REFERENCING_COL_NAME"); // SELF_REFERENCING_COL_NAME String => name of the
        // designated "identifier" column of a typed table
        // (may be null)
        String referenceGeneration =
            rs.getString("REF_GENERATION"); // REF_GENERATION String => specifies how values in
  ...
  public void getColumns(DatabaseMetaData metaData) throws SQLException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        String tableSchema = rs.getString("TABLE_SCHEM"); // TABLE_SCHEM String => schema name
        String tableName = rs.getString("TABLE_NAME"); // TABLE_NAME String => table name
        String columnName = rs.getString("COLUMN_NAME"); // COLUMN_NAME String => column name
        Integer dataType = rs.getInt("DATA_TYPE"); // DATA_TYPE int => SQL type from java.sql.Types
        String typeName =
            rs.getString(
                "TYPE_NAME"); // TYPE_NAME String => Data source dependent type name, for a UDT the
        // type name is fully qualified
        Integer columnSize = rs.getInt("COLUMN_SIZE"); // COLUMN_SIZE int => column size.
        Integer decimalDigits =
            rs.getInt(
                "DECIMAL_DIGITS"); // DECIMAL_DIGITS int => the number of fractional digits. Null is
        // returned for data types where DECIMAL_DIGITS is not
        // applicable.
        Integer numericPrecicionRadix =
            rs.getInt("NUM_PREC_RADIX"); // NUM_PREC_RADIX int => Radix (typically either 10 or 2)
        Integer nullable = rs.getInt("NULLABLE"); // NULLABLE int => is NULL allowed.
        String remarks =
            rs.getString("REMARKS"); // REMARKS String => comment describing column (may be null)
        String columnDefinition =
            rs.getString(
                "COLUMN_DEF"); // COLUMN_DEF String => default value for the column, which should be
        // interpreted as a string when the value is enclosed in single
        // quotes (may be null)
        Integer characterOctetLength =
            rs.getInt(
                "CHAR_OCTET_LENGTH"); // CHAR_OCTET_LENGTH int => for char types the maximum number
        // of bytes in the column
        Integer ordinalPosition =
            rs.getInt(
                "ORDINAL_POSITION"); // ORDINAL_POSITION int => index of column in table (starting
        // at 1)
        String isNullable =
            rs.getString(
                "IS_NULLABLE"); // IS_NULLABLE String => ISO rules are used to determine the
        // nullability for a column.
        String scopeCatalog =
            rs.getString(
                "SCOPE_CATALOG"); // SCOPE_CATALOG String => catalog of table that is the scope of a
        // reference attribute (null if DATA_TYPE isn't REF)
        String scopeSchema =
            rs.getString(
                "SCOPE_SCHEMA"); // SCOPE_SCHEMA String => schema of table that is the scope of a
        // reference attribute (null if the DATA_TYPE isn't REF)
        String scopeTable =
            rs.getString(
                "SCOPE_TABLE"); // SCOPE_TABLE String => table name that this the scope of a
        // reference attribute (null if the DATA_TYPE isn't REF)
        Short sourceDataType =
            rs.getShort(
                "SOURCE_DATA_TYPE"); // SOURCE_DATA_TYPE short => source type of a distinct type or
        // user-generated Ref type, SQL type from java.sql.Types (null
        // if DATA_TYPE isn't DISTINCT or user-generated REF)
        String isAutoIncrement =
            rs.getString(
                "IS_AUTOINCREMENT"); // IS_AUTOINCREMENT String => Indicates whether this column is
        // auto incremented

        String isGeneratedColumn =
            rs.getString(
                "IS_GENERATEDCOLUMN"); // IS_GENERATEDCOLUMN String => Indicates whether this is a
        // generated column
  ...
  public void getIndices(DatabaseMetaData metaData, boolean approximate) throws SQLException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        Boolean nonUnique =
            rs.getBoolean(
                "NON_UNIQUE"); // NON_UNIQUE boolean => Can index values be non-unique. false when
        // TYPE is tableIndexStatistic
        String indexQualifier =
            rs.getString(
                "INDEX_QUALIFIER"); // INDEX_QUALIFIER String => index catalog (may be null); null
        // when TYPE is tableIndexStatistic
        String indexName =
            rs.getString("INDEX_NAME"); // INDEX_NAME String => index name; null when TYPE is
        // tableIndexStatistic
        ...
        Short ordinalPosition =
            rs.getShort(
                "ORDINAL_POSITION"); // ORDINAL_POSITION short => column sequence number within
        // index; zero when TYPE is tableIndexStatistic

        String columnName =
            rs.getString("COLUMN_NAME"); // COLUMN_NAME String => column name; null when TYPE is
        // tableIndexStatistic

        String ascOrDesc =
            rs.getString(
                "ASC_OR_DESC"); // ASC_OR_DESC String => column sort sequence, "A" => ascending, "D"
        // => descending, may be null if sort sequence is not supported;
        // null when TYPE is tableIndexStatistic

        Long cardinality =
            rs.getLong(
                "CARDINALITY"); // CARDINALITY long => When TYPE is tableIndexStatistic, then this
        // is the number of rows in the table; otherwise, it is the number
        // of unique values in the index.

        Long pages =
            rs.getLong(
                "PAGES"); // PAGES long => When TYPE is tableIndexStatistic then this is the number
        // of pages used for the table, otherwise it is the number of pages used
        // for the current index.

        String filterCondition =
            rs.getString(
                "FILTER_CONDITION"); // FILTER_CONDITION String => Filter condition, if any. (may be
        // null)
        ...
        if (index == null) {
          index =
              new Index(
                  tableCatalog, tableSchema, tableName, nonUnique, indexQualifier, indexName, type);
  ...
  public void getPrimaryKey(DatabaseMetaData metaData) throws SQLException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        Short keySequence =
            rs.getShort(
                "KEY_SEQ"); // KEY_SEQ short => sequence number within primary key( a value of 1
        ...
        if (primaryKey == null)
          primaryKey = new PrimaryKey(tableCatalog, tableSchema, tableName, primaryKeyName);

        primaryKey.columnNames.add(keySequence - 1, columnName);
      }
