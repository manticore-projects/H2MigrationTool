...
public class H2MigrationUI extends JFrame {
  ...
  public static final ImageIcon DIALOG_CANCEL_16_ICON =
      new ImageIcon(
          ClassLoader.getSystemResource("com/manticore/icons/16/stop.png"), "Cancel the Dialog.");
  ...
  public static final ImageIcon SEARCH_16_ICON =
      new ImageIcon(ClassLoader.getSystemResource("com/manticore/icons/16/search.png"), "Search.");

  public static final ImageIcon MAIL_NEW_16_ICON =
      new ImageIcon(
          ClassLoader.getSystemResource("com/manticore/icons/16/mail-read.png"), "Search.");
  ...
  public static JDialog getWorkerWaitDialog(Component component) {
    ...
    JDialog dialog =
        new JDialog(windowAncestor, "Operation in progress", Dialog.ModalityType.APPLICATION_MODAL);
  ...
  private final Action helpAction =
      new AbstractAction("Help") {
        ...
        public void actionPerformed(ActionEvent ae) {
          ErrorDialog.show(
              H2MigrationUI.this, new UnsupportedOperationException("Not supported yet."));
        }
      };
        ...
        public void actionPerformed(ActionEvent ae) {
          ...
          if (databaseFileList.getModel().getSize() > 0 && selectedIndices.length == 0)
            ...
            result =
                JOptionPane.showConfirmDialog(
                    H2MigrationUI.this,
                    "Migrate only the " + selectedIndices.length + " selected H2 Database Files?",
                    "Migration of the H2 Databases",
                    JOptionPane.YES_NO_OPTION,
                    JOptionPane.QUESTION_MESSAGE,
                    DIALOG_QUESTION_64_ICON);
                  ...
                  protected List<File> doInBackground() throws Exception {
                    ...
                    for (final File f : databaseFiles) {
                      try {
                        ...
                        String upgradeOptions = quirksModeBox.isSelected() ? "QUIRKS_MODE" : "";
                        if (varbinaryBox.isSelected())
                          upgradeOptions +=
                              upgradeOptions.isEmpty() ? "VARIABLE_BINARY" : " VARIABLE_BINARY";

                        String compression = (String) compressionBox.getSelectedItem();
        ...
        public void actionPerformed(ActionEvent ae) {
          ...
          if (databaseFileList.getModel().getSize() > 0 && selectedIndices.length == 0)
            ...
            result =
                JOptionPane.showConfirmDialog(
                    H2MigrationUI.this,
                    "Verify only the " + selectedIndices.length + " selected H2 Database Files?",
                    "Verification of the H2 Databases",
                    JOptionPane.YES_NO_OPTION,
                    JOptionPane.QUESTION_MESSAGE,
                    DIALOG_QUESTION_64_ICON);
            ...
            SwingWorker worker =
                new SwingWorker() {
                  ...
                  protected Object doInBackground() throws Exception {
                    ...
                    DriverRecord driverRecord = fromVersionList.getSelectedValue();
                    ...
                    for (File f : databaseFiles) {
                      ...
                      try (Connection con = driver.connect("jdbc:h2:" + fileName, properties)) {
                        Collection<Recommendation> r = MetaDataTools.verifyDecinmalPrecision(con);
                        if (!r.isEmpty())
                          if (!recommendations.containsKey(f)) recommendations.put(f, r);
                          else recommendations.get(f).addAll(r);
                      }
                    }
                  ...
                  protected void done() {
                    ...
                    textArea.setFont(MONOSPACED_FONT);
                    for (Entry<File, Collection<Recommendation>> e : recommendations.entrySet()) {
                      textArea.append(
                          "------------------------------------------------------------------------------------------------\n");
                      textArea.append("-- ");
  ...
  public void buildUI(boolean visible) {
    ...
    Color selectedTextBackGround =
        UIManager.getLookAndFeel().getDefaults().getColor("Table[Enabled+Selected].textBackground");
    Color selectedTextForeGround =
        UIManager.getLookAndFeel().getDefaults().getColor("Table[Enabled+Selected].textForeground");
